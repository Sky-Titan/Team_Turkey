...
public class board_subject_write extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        subjectNumber = intent_sw.getExtras().getString("subjectNumber");// intent로 board_subject로부터 subjectNumber 값을 가져옴
        Log.i("b_s_w",subjectNumber);
    }

    public void bsw_add(View view)
    {
        String text4err="";
        if(bsw_title.length()>0&&bsw_content.length()>0&&bsw_password.length()>0) {
            ...
            task.execute("http://119.201.56.98/insert_post.php",subjectNumber,title,content,password);//php수정필요함
